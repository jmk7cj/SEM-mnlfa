...
data_cfa$study_id <- as.numeric(as.factor(data_cfa$study_id))-1
...
out_cfa_study0 <- readModels("cfa_study0.out")
...
out_cfa_study0$summaries
out_cfa_study0$summaries[c("Parameters", "ChiSqM_DF", "RMSEA_Estimate", 
                           "CFI", "TLI", "SRMR")]
...
table_2 <- table_2[rep(1:nrow(table_2), each = 4), ]
table_2[c(seq(1,40, 4)+1, seq(1,40, 4)+2, seq(1,40, 4)+3), ] <- c("") # NA # c("")
...
merged_data$study_id <- factor(merged_data$study_id)
merged_data$sex <- factor(merged_data$sex)
merged_data$race <- factor(merged_data$race)

levels(merged_data$study_id) <- c("FAST", "LIFT", "PIRC1", "PIRC2", "SAFE")
levels(merged_data$sex) <- c("female", "male")
levels(merged_data$race) <- c("black", "white")
